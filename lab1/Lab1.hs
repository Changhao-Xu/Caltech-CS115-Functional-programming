...
--5 -- corrected by rewriting sum according to the definition
{-
 dot = (sum .) . map2 (*)
 (sum .) . map2 (*) lst1 lst2                           -- . operator has high precedence (9) but function application is higher
 (\x -> sum . x) . map2 (*) lst1 lst2                   -- g = map2 (*) lst1 lst2 , due to definition above
